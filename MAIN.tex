...
% RF
On average, RF is the the best-performing models for this
task. 
...
The remaining features have similarimportance values, except
...
observations.
Moreover, by using
balanced inputs higher average results were achieved always, except
when using balanced inputs and min\textunderscore obs = 5, where the
generated scores were very similar. 
...
best model. 
HPM is by a ~20\% difference the class with the most correctly
...
% Best Clf
The best algorithsm is RF with an f1-score of 75.05\% achieved with 
curves containing at least 10 observations, using 30 features and
unbalanced inputs.   

% RF
Overall, RF is alway the best model, always performing better with unbalanced
inputs.  
The worst results are obtained using 20 features only.
Finally, using light curves with a minimum of 5 minimum of 10
observations resulted in better scores when using more than 20
features;  the opposite is true when using 5 observations minimum.   
...
Using balanced inputs also results in slightly bettwe results. 
...
Non-Transients (Table \ref{Overall-Scores-6-Transient-Oversampled}). 
...
stetson\textunderscore j index with a ~9\% importance, followed by
...
Additionally, using balanced inputs works best with light
curves that had 5 observations minimum, whereas unbalanced inputs
...
Additionally, most incorrectly classified AGNs (~20.5\%) are
...
Table \ref{Overall-Scores-8-Class-Oversampled} summareize the results
